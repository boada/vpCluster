...
im=mrdfits(file, 1, header0) ;read in first extension which contains the raw image data (spectra by fiber number)
...
if (testing ne 1) then begin
    readcol,'badfibers.txt',bad_fibers, FORMAT='F'
endif
...
wave_pix1=100 ;beginning of the spectrum that is just noise, changes if cdelt changes.
wave_pix2=250 ;set our window to be 150 wavelength pixels wide

if (testing ne 1) then begin
    ;This is our check of what's actually noise
    window,4,xsize=1200,ysize=700
    print, size(im)
    plot,fiber,total(im[wave_pix1:wave_pix2,*],1),yrange=[0.0,100.0],xtitle="fibre number",ytitle="summed counts",title="Nrd Determination" ;summed counts is between assigned wavelengths
...
if (testing ne 1) then begin
    fileout='./cube.fits'
    fileout2='./cube_with_sky.fits'
    fileout3='./cube_var.fits'
    diagout='./diagnostic.fits'
endif
