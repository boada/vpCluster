...
y_ = astStats.runningStatistic(pyl.log10(target['M200c']),
        pyl.log10(target['MASS']), pyl.percentile, binNumber=20, q=[16, 50, 84])
...
print('3d')
y_ = astStats.runningStatistic(pyl.log10(target['M200c']), target['ML_pred_3d'],
        pyl.percentile, binNumber=20, q=[16, 50, 84])
quants = pyl.array(y_[1])
ax1.plot(y_[0],quants[:,1], '--', c=c, zorder=zo)
if not c == '#e24a33':
...
err = calc_err(10**target['ML_pred_3d'], target['M200c'])
y_ = astStats.runningStatistic(pyl.log10(target['M200c']), err,
        pyl.percentile, binNumber=20, q=[16, 50, 84])
quants = pyl.array(y_[1])
ax1s.plot(y_[0],quants[:,1], '--', c=c, zorder=zo)
...
ax1.legend([line1, line2], ['Power Law', '$ML_{\sigma, z, Ngal}$'], loc=2)
