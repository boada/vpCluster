...
ax.errorbar(
    pyl.log10(rozo['lambda']),
    pyl.log10(rozo['mass'] * 1e15),
    xerr=mkError(rozo['lambda'], rozo['lambda_err']),
    fmt='o',
    label='Rozo+2010')
...
ax.errorbar(
    pyl.log10(sifonRichness),
    pyl.log10(sifonMasses),
    yerr=mkError(sifonMasses, sifonMass_err),
    fmt='o',
    color='#7a68a6',
    label='Sifon+2015')
...
print("m = {0} +/- {1}".format(
    pyl.mean(samples[:, 0]), pyl.std(samples[:, 0])))
print("b = {0} +/- {1}".format(
