...
def findSeperationSpatial(data, center):
    ...
    for row in data.iterrows():
        ...
        #data['seperation'][row[0]] = sepDeg * 3600
...
def rejectInterlopers_group(data, sigmav=500):
    ...
    deltaRmax = (c * deltaZmax)/(10*(1 + avgz)*aca.H0*aca.Ez(avgz)) # Mpc

    # make redshift cut
    data = data[abs(data['redshift']-avgz) <= deltaZmax]

    # make spatial cut
    data = data[data['seperation'] <= deltaRmax]

    return data

catalog = '/Users/steven/Projects/cluster/data/boada/may_2013/catalogs/XMMXCSJ125650.2+254803.2_complete.csv'

files = glob.glob('*.results')
center = 194.208333333, 25.8008333333
...
cleaned = rejectInterlopers_group(seperated)

xsort = cleaned.sort('redshift', ascending=True)
difference = pyl.diff(xsort['redshift'])
s = 1.135*c*pyl.sqrt(pyl.pi)/(len(cleaned)*(len(cleaned)-1))

print s*sum([i*(len(cleaned)-i)*g for i,g in enumerate(difference)])
